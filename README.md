...
Add the fact that the browser engine lives **outside your app** - the system's WebKit by default, an exact signature-verified build from the shared engine store when you need byte-for-byte consistency, our own from-source WinCairo on Windows - and your updates stop being hostage situations, engine security fixes stop being your re-release problem, and your laptop fan gets a long-overdue vacation. Buildless native extensibility plus no engine tax - that's the part no amount of tree-shaking inside a bundled Chromium can give you.
