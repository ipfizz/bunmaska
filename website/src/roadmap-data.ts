...
export const milestones: readonly Milestone[] = [
  ...
  },
  {
    ...
    note: 'the foundation the alpha.7 dev-loop work stood on.',
    items: [
  ...
  },
  {
    version: 'alpha.7',
    status: 'shipped',
    title: 'The dev loop rebuilt, and updates you can ship',
    note: 'where we are today - the daily loop and the update pipeline both got real.',
    items: [
      'Bunmaska owns the renderer build: a `renderer` block in the config, dev rebuilds + live-reloads a React edit instead of restarting the app, and `build` ships the bundle beside the executable on all three OSes.',
      'A dev loop that respects your saves: content-hashed change detection (no-op saves do nothing), atomic editor saves never lost, exit-awaited restarts, preload edits restart, the engine pin honored by dev/run, and window bounds preserved across restarts.',
      'Signed auto-updates end to end: `bunmaska keygen`, `build --update --update-key` emitting a detached Ed25519 `.sig`, `setFeedURL({ url, publicKey, channel })` over https only, unsigned feeds refused, and a real `quitAndInstall` swap-and-relaunch. `--notarize` submits and staples for real.',
      'API depth pulled forward from the alpha.8 list: `session.cookies` on macOS + Linux, Linux `capturePage`, real macOS frame geometry (`getBounds` in top-left coordinates, a `move` event), `setApplicationMenu(null)`, Super=Cmd accelerators, named `bunmaska/electron` exports.',
      'What did NOT ship here, honestly: the hosted-engine catalogue work (stable-train WinCairo, the hosted Linux engine, CI publishing) moved to alpha.8 - the feed itself has been live since alpha.6.',
    ],
  },
  {
    version: 'alpha.8',
    status: 'next',
    title: 'Hosted engines + the last API stretch',
    note: 'the distribution unlock, plus the gap list the parity page still admits to.',
    items: [
      'The engine catalogue: the stable-train WinCairo build (hosted engines track WPE stable releases, where security advisories land), the hosted Linux engine, and publishing wired into CI instead of run by hand. The feed and trust anchor are live already.',
      'The render pass for the relocatable Linux engine: it loads from the store today; next it draws.',
      'Cross-distro Linux engines (an old-glibc base, one build across distros) and engine delivery for end users: embedded in the bundle or fetched on first run. Never something a user types.',
      '`powerMonitor` idle/battery, `page-title-updated`, per-instance `webContents.ipc`, `session.clearStorageData` on Linux.',
      'The self-pipe wakeup primitive that structurally prevents the one hang class we’ve ever shipped, and a worked serial-port native module - the buildless-FFI flagship demo, as a real package.',
    ],
    exit: 'a fresh machine goes install > init > build > running app on a pinned engine, on all three OSes; no "easy" cells left on the parity page',
  },
  {
