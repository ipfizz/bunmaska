...
It's **alpha**. It genuinely works on **macOS, Linux, and Windows** today - real windows, WebKit rendering on all three (system framework on macOS/Linux, a from-source WinCairo build on Windows), Electron-style IPC with context isolation, menus, tray, dialogs, clipboard (incl. images), `nativeImage`, `safeStorage`, `powerMonitor`, and a CLI that packages real `.app`/`.deb`/`.exe` distributables (plus `.dmg` with a flag). A few APIs differ per platform - `printToPDF` is macOS-only, `capturePage` is macOS + Linux, and those two plus custom `protocol://` are engine-blocked on Windows - and the [parity matrix](/docs/migrating/parity) is honest about every cell. Some method-level corners are still being filled in.
